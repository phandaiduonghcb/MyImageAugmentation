...
class RandomCrop(object):
    def __init__(self,p=0.5,w=40,h=40):
        self.p = p
        self.w =w
        self.h=h
    def __call__(self,img,bboxes):
        ...
        if type(self.h) is tuple:
            ...
            h = self.h
        for bbox in bboxes:
            S = (bbox[3]-bbox[1])*(bbox[2]-bbox[0])
            S_crop = S*0.2
            canhngang_crop = int(random.randint(min_w,w)/100*(bbox[2]-bbox[0]))
            canhdoc_crop = int(random.randint(min_h,h)/100*(bbox[3]-bbox[1]))
            x1 = random.randint(int(bbox[0]),int(bbox[2]-canhngang_crop))
            y1 = random.randint(int(bbox[1]),int(bbox[3]-canhdoc_crop))
            x2 = x1 + canhdoc_crop
            y2 = y1 + canhdoc_crop
            for i in range(y1,y2):
                img[i][x1:x2] = [np.zeros(3)] * img[i][x1:x2].shape[0]
        return img,bboxes
